...
**Companion specifications:** [Agent Skill architecture](agent-skill-architecture.md) · [Agent and PWA UX](ui-design.md) · [Vendor evidence and testing research](research/vendor-evidence-and-testing-plan.md)
...
This policy governs the planning documents and vendor research plan, future rule packs, reports, adapter descriptions, UI copy, and benchmark publications.
...
| **Authorized vendor observation** | Require a documented testing grant, dated/versioned method, fixture counts, observable output, and exact tenant/workflow scope. Publication permission is independent of access/testing permission. Sandbox availability alone is neither a grant to CV Linter nor a measurement. |
| **Local parser compatibility check** | Describe CV Linter's own extraction/rule observation, tied to its input, parser, and evidence. Local benchmark results support local behavior only; they do not validate a named ATS. |
...
**Evidence hierarchy:** Official product/support/API documentation is the authority for documented vendor capabilities. Authorized vendor experiments support observed behavior only in their tested scope. Independently annotated local benchmarks support our parser and rules. Published studies support their own evaluation settings. Candidate-facing best-practice blogs, including vendor-hosted career advice, are **hypothesis sources**, not authority for ATS requirements, scoring weights, or rejection claims. This is authority by claim type, not one confidence ladder: local success never upgrades a vendor claim. Keep facts, measurements, and the inference connecting them separate. The [vendor research plan](research/vendor-evidence-and-testing-plan.md#1-recommendation-and-evidence-hierarchy) specifies the hierarchy and permission gates.
...
The earlier planning pass records source checks on 6 September 2026 for Agent Skills, host documentation, MCP tools, key vendor pages and judge papers, with additional Agent Skills, MCP and provider-retention checks in that revision. The 8 September vendor proposal review adds a [separate research document](research/vendor-evidence-and-testing-plan.md) covering Teamtailor, Greenhouse, Lever, and Workable, including access routes and selected public terms. These are selective documentation checks, not tenant testing or exhaustive recertification. Lever's accessible API documentation independently confirms image attachment versus parsing behavior, but some help pages still fail to load; their broader UI format/search claims remain **memo-attributed, pending source recheck**. Ashby and iCIMS' accented-email limitation retain their earlier pending status. Do not invent publication or last-verification dates for inherited claims. Workday and Taleo refinements below preserve information found in the earlier primary-page review; this vendor pass did not recheck those profiles or judge research.
...
**Research priority:** Teamtailor, Greenhouse, Lever, and Workable form the first access-and-evidence cohort. The other inherited profiles remain background guidance, with their citations and unresolved questions preserved. The [vendor research plan](research/vendor-evidence-and-testing-plan.md#2-research-snapshot-and-source-status) is the detailed source inventory; its [access and permission register](research/vendor-evidence-and-testing-plan.md#3-access-and-permission-register) separates possible access routes from grants held by CV Linter. No vendor tests or access grants exist in this revision.

| Vendor | Vendor facts and scope | Design inference and limits |
|---|---|---|
| **Teamtailor** | Co-pilot documents personal details/profile image, work/education history, skills/traits tied to the account library, and hidden-instruction warnings that do not change the score. [Parser](https://support.teamtailor.com/en/articles/16127956-co-pilot-resume-parser). Search supports `resume_text`; screening uses configured criteria and multiple candidate/application inputs. [Search/export](https://support.teamtailor.com/en/articles/6121249-filter-segment-and-export-candidates), [screening](https://support.teamtailor.com/en/articles/10209597-co-pilot-candidate-screening). | Separate base parsing, Co-pilot enrichment, search, and screening. Screening's format list is not a universal upload/parser contract. No anonymous parser endpoint was located. Seek an approved Co-pilot-enabled sandbox and inspectable output before claiming tested behavior; hidden-text observations do not establish intent or rejection. |
...
| **Lever** | The memo attributes parsing support for Word/DOCX, PDF, RTF, WordPerfect, HTML/MS Office HTML, and ODF, but not JPG/PNG, with profile-field population. It distinguishes resume-only from broader attachment/notes/feedback search; these help-page claims remain pending source recheck. [Parsing](https://help.lever.co/hc/en-us/articles/20087345054749-Understanding-resume-parsing), [Resume search](https://help.lever.co/hc/en-us/articles/20087317030685-Searching-the-Database-for-Candidates), [advanced search](https://help.lever.co/hc/en-us/articles/20087212721309-Using-advanced-search-and-rediscovery). The rechecked API confirms that image files can be attached without parsing and describes authenticated opportunity creation with parsing. [API](https://hire.lever.co/developer/documentation). | Check native-text availability and whether terms survive extraction; local checks do not establish Lever search results. Do not transfer the UI format list to the API. Partner sandbox access requires approval, and published terms restrict benchmarking; obtain an express agreement for the study and separate publication permission. [Partner process](https://hire.lever.co/developer/partner), [terms](https://www.lever.co/legal/terms-of-service). |
| **Workable** | Recruiter upload guidance documents image-based and multilingual CV parsing. [Parsing/uploads](https://help.workable.com/hc/en-us/articles/115012661408-Uploading-candidate-resumes-CVs-Individual-and-bulk-options). Application resumes support PDF, DOC/DOCX, RTF, HTML and ODT with a 5 MB limit, separately from custom-question attachments. [File types](https://help.workable.com/hc/en-us/articles/115012238108-What-types-of-files-can-be-uploaded-on-the-application-form). Search covers resume/profile sections; configured Yes/No responses can cause disqualification. [Search](https://help.workable.com/hc/en-us/articles/6058530293015-How-do-I-run-an-advanced-boolean-search-for-candidates), [screening](https://help.workable.com/hc/en-us/articles/115012238688-Auto-disqualify-candidates-using-application-form-questions). | Local lack of OCR is a CV Linter limitation, not proof of Workable failure. Image-based parsing does not establish JPG/PNG acceptance in the application resume field or measured accuracy in every language. A documented trial with fake candidates and an approved-partner sandbox route do not establish automated benchmark or publication permission. |
...
Each future rule resolves **`claim_kind`, `scope`, `source_url`, `verification_status`, `testing_permission`, `last_verified_at`, `benchmark_status`, and `evidence_level`**, plus source title/section, publication date if known, configuration prerequisites, unit semantics, rule version, applicability, and evidence references. The [proposed metadata contract](research/vendor-evidence-and-testing-plan.md#4-architecture-and-rule-metadata) defines their meanings. A local detector, its cited vendor premise, and its inferred downstream consequence need separate linked claims. Testing permission records synthetic uploads, automation, and publication independently; benchmark status records local and vendor coverage separately. Source review is not a test, a local benchmark cannot raise vendor evidence, and one tested claim does not validate an entire profile.

Do not convert ambiguous MB/KB or pre-/post-Base64 limits into exact byte rejection thresholds without verifying semantics; near-boundary cases remain cautions until resolved. Updates create new profile versions; analysis never fetches updated sources or changes an existing report's profile silently. Preserve conflicting documentation and observations with their dates/scopes. Unreadable sources stay pending recheck with a nullable last-verification date.
...
For each of the four priority vendors: inventory official parser/file-format/search/screening sources → identify account/API/trial/partner/test-tenant routes → resolve synthetic-testing, automation, and publication rights → agree a paired PDF/DOCX pilot with observable fields → measure stages separately → publish only permitted, scoped findings → version and recheck claims. Record unavailable sources, fields, features, or permissions as unknowns. See the [detailed workflow](research/vendor-evidence-and-testing-plan.md#5-vendor-research-workflow-and-benchmark-design).

**Next research action:** The project owner should send the [draft Teamtailor support/partner request](research/vendor-evidence-and-testing-plan.md#6-proposed-teamtailor-supportpartner-request--unsent) for an approved sandbox, synthetic PDF/DOCX uploads, inspectable parsed fields, API/export access, automation/rate limits, and scoped publication permission. The request is prepared but unsent. Continue local fixture planning and the remaining source gaps while access is unresolved; do not make vendor access a prerequisite for the local MVP.
...
7. Source citation or benchmark evidence supporting each claim, with claim kind, scope, verification/evidence status, and local versus vendor benchmark coverage available in report details.
...
Keep the vendor source/claim registry, permission register/vendor experiment archive, and local parser benchmark evidence separate, joined by stable references. The executable and PWA consume pinned reviewed profile data; selecting a vendor never uploads a CV or grants account access. A future research-only vendor harness sits outside normal product commands and uses only an approved tenant/workflow and testing scope. It is not an MVP dependency. Reviewed findings enter a new profile version, with credentials and private agreements excluded from distributed artifacts.
...
Follow the [vendor research and testing plan](research/vendor-evidence-and-testing-plan.md), starting with Teamtailor, Greenhouse, Lever, and Workable. Keep three evaluation tracks distinct: official documentation review, approved vendor experiments, and local parser compatibility checks against independent gold annotations. Local success, an API response, or a sandbox invitation does not establish vendor compatibility.

Before an external experiment, record the authorized tenant, actual parser-triggering workflow and available fields, synthetic fixture grant, automation/rate/volume limits, applicable terms, and publication scope. Permission to test and permission to publish are independent. Lever's published benchmarking restriction requires an express applicable agreement; other unresolved permissions remain unknown. General mock-data examples or partner development access do not authorize our proposed benchmark.

Once authorized, submit controlled paired fixtures and record:
...
- Fixture/gold-label versions and counts, run IDs, permitted workflow, feature flags, and parser version if exposed (otherwise explicitly unknown).
- Whether fields came from parsing, supplied API data, deduplication, enrichment, or manual edits; distinguish raw parser output from profile/UI/export observations.
- Completion/indexing waits, timeouts, repeats, missing observability, and uncertainty. Keep test search terms out of unrelated profile data.
...
Until reviewed experiments exist, describe vendor profiles as **documentation-based guidance**. Only the observed claim/workflow can gain measured status, with publication gated separately; no vendor-wide certification follows. Local MVP work continues if access is unavailable or denied. Do not run experiments through live job applications.
...
|---|---|---|
| **0 — Contracts, evidence and feasibility** | Claims policy and rule metadata; four-vendor official-source/access/permission register; proposed Teamtailor request; executable/API and judge confirmation contracts; exact-payload/ZDR metadata; first 20 annotated fixtures; parser/runtime/model spikes; host invocation check; preregister benchmarks | Supported platforms/input limits, model candidates, explicit documentation/local/vendor evidence boundaries and testable contracts; unresolved vendor grants do not block local work; no MCP or vault prerequisite |
...
| **4 — Validated beta** | Locked local parser/rule/judge/workflow benchmarks; usability study; source/profile review; separate authorized vendor pilot when access and terms permit | Publish methods, results and limitations within publication rights; distinguish local validation from each tested vendor scope; untested vendor claims remain documentation-based; a validated local judge task remains the target, with unmet gaps explicit |
...
| API/sandbox access is mistaken for research rights or empirical evidence | Separate synthetic/automation/publication grants, terms restrictions, local/vendor benchmark coverage, and per-claim evidence levels; continue local work without vendor grants |
...
1. Use the main plan, companion specifications, and vendor research plan to create MVP tickets for the executable/skill, fixtures, parser evidence, package verification, judge adapters and one review/confirmation flow; track vendor access/outreach separately and keep MCP and durable history deferred. The immediate documentation follow-up is the unsent Teamtailor request in section 3, not vendor integration code.
...
4. Specify the first ten rules with the eight evidence metadata fields in section 3, applicability, abstention, and correction guidance; distinguish local observations, documented vendor premises, and inferred consequences.
