...
The earlier proposed **time-boxed extraction bake-off** between excoffierleonard/parser and upstream ParseKit is a historical research note, superseded by the Xberg MVP parser decision. Any retained results are a local extraction study only; they must not be described as an ATS compatibility test. Keep versions, fixture IDs, results, limitations, and license decisions with the historical spike record.
